...
function stat(value: unknown): value is ProgressStat {
  ...
  return (
    number(value.attempts, true) &&
    number(value.correct, true) &&
    value.correct <= value.attempts &&
    number(value.total) &&
    number(value.best) &&
    number(value.last, true) &&
    Array.isArray(value.recent) &&
    value.recent.length <= 6 &&
    value.recent.every((item) => typeof item === 'boolean') &&
    optional(
      value.box,
      (item) => number(item, true) && item >= 1 && item <= 5,
    ) &&
    optional(value.intervalDays, (item) => number(item) && item <= 30) &&
    optional(value.dueAt, (item) => number(item, true)) &&
    optional(
      value.lapses,
      (item) => number(item, true) && item <= (value.attempts as number),
    )
  );
}
function attempt(value: unknown): value is ProgressAttempt {
  return (
    ...
    text(value.id, 200) &&
    value.id.length > 0 &&
    !unsafeKeys.has(value.id) &&
    text(value.topic, 100) &&
    value.topic.length > 0 &&
    text(value.q) &&
    text(value.a) &&
    typeof value.correct === 'boolean' &&
    number(value.ms) &&
    number(value.at, true) &&
...
export function isSavedProgress(value: unknown): value is SavedProgress {
  ...
  return (
    optional(value.resetAt, (item) => number(item, true)) &&
    optional(
      value.stats,
      (items) =>
        record(items) &&
        Object.keys(items).length <= 10000 &&
    ...
    optional(
      value.history,
      (items) =>
        Array.isArray(items) && items.length <= 1500 && items.every(attempt),
    ) &&
